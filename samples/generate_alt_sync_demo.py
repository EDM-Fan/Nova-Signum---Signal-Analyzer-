"""
samples/generate_alt_sync_demo.py
=================================
Generates a complete BPSK test packet exercising alternate sync word detection (Alt A / CCSDS = 0x1ACFFC1D):
1. 512 random information bits
2. Convolutional Encoding (K=7, Rate 1/2) -> 1036 coded bits (512 + 6 tail)
3. Pad with dummy bits to 1280 (5 full 256-bit blocks)
4. Block Interleaving (8 x 32) -> 1280 interleaved bits
5. Prepend 32-bit alternate sync word (SYNC_CCSDS_32 = 0x1ACFFC1D) -> 1312 packet bits
6. Modulate as BPSK baseband IQ (RRC pulse shaping, sps=8, Fs=1MHz, SNR=18dB, CFO=0Hz)
7. Saves float32 .iq file to samples/bpsk_sync_alt_demo.iq
8. Saves ground-truth bits to samples/bpsk_sync_alt_demo_bits.npy
"""
...
def generate_alt_sync_demo(
    ...
    # 5. Prepend unencoded 32-bit alternate sync word (Alt A: 0x1ACFFC1D)
    sync_word = SYNC_CCSDS_32.copy()
    ...
    iq_path = os.path.join(OUTPUT_DIR, "bpsk_sync_alt_demo.iq")
    ...
    npy_path = os.path.join(OUTPUT_DIR, "bpsk_sync_alt_demo_bits.npy")
    np.save(npy_path, info_bits)

    print("Generated alternate sync demo packet:")
    print(f"  Sync Word        : Alt A / CCSDS (0x1ACFFC1D) [32 bits]")
